import { matchMinutesDelayed } from "./identify-delay.mjs"
...
function addDelay(changes, text) {
  let delay = matchMinutesDelayed(text)
  if (delay) changes.push(delay)
...
export default function identifyChanges(text, serviceData, networkData) {
  ...
  if (firstChange) {
    ...
    return addDelay(changes.map(change => {
      return {
        type: change.type,
  ...
  if (firstChange) {
    ...
    let data = [{
      type: isUp ? 'originate' : 'terminate',
      location: cleanedLocations.find(loc => loc !== (isUp ? serviceData.origin : serviceData.destination))
    }]
    
    return addDelay(data, text)
  }
}